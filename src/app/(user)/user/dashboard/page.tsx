...
export default function CustomerDashboardPage() {
  ...
  const loading = winsLoading || auctionsLoading;
  const accessedAuctions = meData?.user?.paidAccessAuctions || [];

  const liveAuctions = auctionsData?.auctions.filter((a) => a.status === "LIVE") || [];
  ...
  const renderCardHeader = (car: { title: string; lotNumber?: string; location?: string }) => (
    <div className="flex-1">
      ...
      <div className="flex justify-between items-center mt-1.5">
        ...
        {car.location && (
          <SpecChip icon="location_on">{car.location}</SpecChip>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8 overflow-x-hidden">
      ...
      {loading ? (
        ...
        <>
          {liveCars.length === 0 && upcomingCars.length === 0 && (
