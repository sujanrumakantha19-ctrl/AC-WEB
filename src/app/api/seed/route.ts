...
export async function POST() {
  try {
    ...
    // High quality vehicle image URLs
    ...
    const imgBrezza = "https://images.unsplash.com/photo-1541899481282-d53bffe3c35d?auto=format&fit=crop&w=1200&q=80";
    ...
    const auctionsData = [
      // 1. LIVE AUCTION
      {
        ...
        rounds: 3,
        roundTimes: [
          { start: new Date(now.getTime() - 60 * 60 * 1000).toISOString(), end: new Date(now.getTime() + 30 * 60 * 1000).toISOString() },
          { start: new Date(now.getTime() + 35 * 60 * 1000).toISOString(), end: new Date(now.getTime() + 90 * 60 * 1000).toISOString() },
          { start: new Date(now.getTime() + 95 * 60 * 1000).toISOString(), end: new Date(now.getTime() + 150 * 60 * 1000).toISOString() },
        ],
        currentRound: 1,
        roundStates: [
      ...
      // 2. LIVE AUCTION
      {
        ...
        rounds: 2,
        roundTimes: [
          { start: new Date(now.getTime() - 40 * 60 * 1000).toISOString(), end: new Date(now.getTime() + 45 * 60 * 1000).toISOString() },
          { start: new Date(now.getTime() + 50 * 60 * 1000).toISOString(), end: new Date(now.getTime() + 110 * 60 * 1000).toISOString() },
        ],
        currentRound: 1,
        roundStates: [
      ...
      // 4. UPCOMING AUCTION
      {
        ...
        verifiedSeller: true,
        inspectionScore: 9.7,
        engine: "2.8L Turbocharged Diesel Engine (500 Nm Torque)",
        ...
        whatsappGroups: [{ link: "https://chat.whatsapp.com/sample-group-4", limit: 900, notifyBefore: 15 }],
      },
      ...
      // 5. UPCOMING AUCTION
      {
        ...
        seller: "BMW Premium Selection Partner",
        verifiedSeller: true,
        inspectionScore: 9.5,
      ...
      // 7. ENDED AUCTION 2
      {
        ...
        verifiedSeller: true,
        inspectionScore: 9.0,
        engine: "1.4L Smartstream T-GDi Petrol",
        color: "Gravity Grey",
        ownership: "1st Owner",
        insurance: "Valid",
    ...
    // 5. Create Offers for Auctions
    const offersData = [
      // Offers for Thar (LOT-1001)
    ...
    // 6. Update Customer Paid Access & Refund Statuses
    // Rajesh Kumar: Paid Thar, Merc, City
    uRajesh.paidAccessAuctions = [aThar._id, aMerc._id, aCity._id];
    await uRajesh.save();

    // Priya Sharma: Paid Creta
    uPriya.paidAccessAuctions = [aCreta._id];
    await uPriya.save();

    // Vikramaditya Singh: Paid Merc, Seltos
    uVikram.paidAccessAuctions = [aMerc._id, aSeltos._id];
    uVikram.refundedAuctions = [aMerc._id]; // Refunded for Merc (since Rajesh won Merc)
    await uVikram.save();

    // Ananya Deshmukh: Paid Thar
    uAnanya.paidAccessAuctions = [aThar._id];
    await uAnanya.save();

    // Suresh Reddy: Paid Nexon
    uSuresh.paidAccessAuctions = [aNexon._id];
    await uSuresh.save();

    // Karthik Venkat: Paid Creta
    uKarthik.paidAccessAuctions = [aCreta._id];
    await uKarthik.save();

    // Arjun Mehta: Paid Thar, Seltos
    uArjun.paidAccessAuctions = [aThar._id, aSeltos._id];
    uArjun.refundedAuctions = [aSeltos._id]; // Refunded for Seltos
    await uArjun.save();
    ...
    return NextResponse.json({
      success: true,
      message: "Database successfully reset and seeded with realistic vehicle auction data",
      counts: {
        admin: 1,
        users: customersData.length,
