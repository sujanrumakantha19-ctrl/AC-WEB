  ...
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-64 h-16 px-5 lg:px-8 bg-white/90 backdrop-blur-xl border-b border-outline-variant/30 z-40 flex items-center justify-between transition-all">
      ...
      <div className="lg:hidden flex items-center gap-2.5">
        ...
        <span className="font-extrabold text-primary tracking-tight text-base">VKS AUTOSERVICES</span>
      </div>
        ...
        <Link href="/user/profile" title="My Profile" className="flex items-center gap-2.5 cursor-pointer group pl-2 border-l border-outline-variant/30 hover:opacity-80 transition-opacity">
          <div className="text-right hidden sm:block">
            {isLoading ? (
              ...
              <>
                <p className="text-xs font-bold text-on-surface leading-tight">{user?.name || "Member"}</p>
                <p className="text-[10px] text-primary font-bold leading-tight mt-0.5 break-all">{user?.cusId || ""}</p>
              </>
            )}
          </div>
          {isLoading ? (
        ...
        <div className="lg:hidden absolute top-16 left-0 right-0 bg-white border-b border-outline-variant/30 p-4 space-y-2.5 shadow-xl z-50">
          ...
          <div className="pt-2 border-t border-outline-variant/20 flex items-center justify-between gap-3">
            <button
              ...
              className="text-left py-2 text-xs font-bold text-error flex items-center gap-3"
            >
